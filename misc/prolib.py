...
aa1_std = rnam1_std
aa3_std = rnam3_std
rnam3_std_dict = dict(zip(rnam3_std, range(len(rnam3_std))))
rnam1_std_dict = dict(zip(rnam1_std, range(len(rnam1_std))))
rnam1_ASA_dict = dict(zip(rnam1_std, ASA_std))
#
...
def rdfasta(fn):
	seq1 = ""; name = ''
	for i,x in enumerate(openfile(fn)):
...
class Parse_seq:
	def __init__(self, fn, bstrip=True, ipos=None):
		self.fp = openfile(fn)
	...
	def proc_seq(self, f1):
		if self.bstrip: return f1.strip()
		else: return f1
	def next(self):
		rec1 = SeqRec()
# parse the head
		if self.lastline is None: raise StopIteration
			...
			if self.ipos is not None:
				ss = re.split(r'[>|:\s]+', info1)
				if ss[0] == '': del ss[0]
				rec1.name = ss[self.ipos]
			else:
				ss = info1.split()
				if len(ss) >= 1: rec1.name = ss[0]
		else:
			rec1.seq = self.proc_seq(self.lastline)
# read seq
		for x in self.fp:
			if x[0] == '>':
				self.lastline = x
